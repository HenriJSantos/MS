...
model.addConstr( == Sensor5,"c8")
...
model.addConstr(Sensor12 <= 3982, "c15")
model.addConstr(Sensor10 <= 2946, "c16")
model.addConstr(Sensor9 <= 4038, "c17")
model.addConstr(Sensor8 <= 4268, "c18")
model.addConstr(Sensor7 <= 3976, "c19")
model.addConstr(Sensor6 <= 3230, "c20")
model.addConstr(Sensor5 <= 2769, "c21")
model.addConstr(Sensor21 <= 1311, "c22")
model.addConstr(Sensor20 <= 1248, "c23")
model.addConstr(Sensor19 <= 1754, "c24")
model.addConstr(Sensor25 <= 2592, "c25")
model.addConstr(Sensor24 <= 2743, "c26")
model.addConstr(Sensor11 <= 3383, "c27")		
...
with open('matrizOD_d_Mon_12.csv', mode='a') as solution_file:
